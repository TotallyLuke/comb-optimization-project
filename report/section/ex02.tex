...
swaps one or more couple of elements in the chromosome. In our implementation a PSM mutation 
...
The genetic algorithm terminates if the maximum number of generations without improvements of the best individual in the population. By default, this limit is set to 100 generations, after which the best individual found so far is returned.
...
An object of class \texttt{Solver} needs the caller to set a \texttt{FitnessEvaluator}, and all the genetic operators (for Mutation, Crossover, and Selection) in order for the \texttt{solve} method to be used on an population (vector) of \texttt{Tour}.
...
To compile and run the program for each part, the user must follow these steps:

\begin{enumerate}
    \item For each $ i $ in \{1, 2\}:
    \begin{enumerate}
        \item Navigate to the directory \texttt{part\textit{i}}:
        \begin{verbatim}
        cd part{i}
        \end{verbatim}
        \item Compile the program using the Makefile:
        \begin{verbatim}
        make
        \end{verbatim}
        \item Run the program with a specific test instance:
        \begin{verbatim}
        ./assignment{i} -f test_instances/FILENAME
        \end{verbatim}
        where \texttt{FILENAME} is the name of the test instance to run.
    \end{enumerate}
\end{enumerate}

\subsection{Example}
For example, to run the program in \texttt{part1} with a test instance named \texttt{random64.tsp}, the user would execute the following commands:

\begin{verbatim}
cd part1
make
./assignment1 -f test_instances/random64.tsp
\end{verbatim}

Similarly, for \texttt{part2}:

\begin{verbatim}
cd part2
make
./assignment2 -f test_instances/random64.tsp
\end{verbatim}

\subsection{Options}
Available options can be displayed by running \texttt{./assignment1 -h}or \texttt{assignment2 -h}. The output for \texttt{./assignment2 -h} is the following:
\begin{verbatim}
assignment2 [options]
options:
...
For \texttt{./assignment1} only the first two options (filename selection and timeout in milliseconds) are available.
...
Any run of \texttt{assignment2 -f FILENAME} generates, for each of the 8 combinations
